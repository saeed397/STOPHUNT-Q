...
class CoinGeckoProvider:
    ...
    def get_simple_price(self, coingecko_id: str, quote: str = "usd") -> float:
        """Fetch a fresh real-time price from CoinGecko simple/price endpoint.
        Documented at: https://docs.coingecko.com/reference/simple-price
        """
        url = "https://api.coingecko.com/api/v3/simple/price"
        params = {"ids": coingecko_id, "vs_currencies": quote.lower()}
        try:
            resp = self.session.get(url, params=params, timeout=10)
            resp.raise_for_status()
            data = resp.json()
            price = data.get(coingecko_id, {}).get(quote.lower())
            if price is None:
                raise ProviderError(f"No price returned for {coingecko_id}")
            return float(price)
        except Exception as e:
            raise ProviderError(f"CoinGecko simple price failed: {e}") from e
