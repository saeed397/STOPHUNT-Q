"""data/validator.py — never fabricate candles. Record gaps only."""
...
def validate_candles(candles: List[Candle], asset: str, timeframe: str,
                     actual_interval_seconds: int = None) -> Tuple[List[Candle], DataQualityReport]:
    if not candles:
    ...
    step = actual_interval_seconds or _timeframe_seconds(timeframe)
    gaps: List[Tuple[int, int]] = []
    for i in range(1, len(clean)):
        expected = clean[i-1].timestamp + step
        actual = clean[i].timestamp
        if actual > expected + step * 0.5:  # allow small jitter
            gaps.append((clean[i-1].timestamp, clean[i].timestamp))

    # Drop still-forming candle
    if clean and not clean[-1].closed:
        clean = clean[:-1]

    report = DataQualityReport(
        asset=asset, timeframe=timeframe, total_candles=len(clean),
        duplicates_removed=dup_count, gaps=gaps, ohlc_violations=violations,
    )
    if len(clean) == 0:
