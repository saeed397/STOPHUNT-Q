...
def calibrate(candles: List[Candle], asset: str, timeframe: str,
    ...
    # Keep the raw historical percentiles. Only apply a tiny floor so that
    # completely zero values (rare) do not produce zero-distance stops.
    # No artificial widening is applied — the asset history decides the gaps.
    p40 = max(p40, 0.05)
    p65 = max(p65, 0.05)
    p80 = max(p80, 0.05)
    ...
    return AssetTimeframeProfile(
        ...
        penetration_depth_atr_p40=p40,
        penetration_depth_atr_p65=p65,
        penetration_depth_atr_p80=p80,
