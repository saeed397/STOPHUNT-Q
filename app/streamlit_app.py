...
Version with forced fresh data + visible run timestamp for debugging.
"""
import sys
import os
import time
...
.signal-title { font-size: 1.1rem; font-weight: 600; margin-bottom: 12px; }
...
.header-price { font-size: 1.4rem; font-weight: 700; }
</style>
...
def main():
    ...
    st.caption("نسخه دیباگ — هر اجرا زمان و منبع داده را نمایش می‌دهد")
    ...
    if not confirm:
        ...
        st.warning("برای دریافت قیمت و سیگنال جدید حتماً دکمه تایید و اجرا را بزنید.")
        return

    # Clear all caches on every run
    st.cache_data.clear()
    st.cache_resource.clear()

    run_time = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
    st.success(f"⏱️ زمان این اجرا: {run_time}")

    provider = MultiProviderOHLC()
    with st.spinner("در حال دریافت داده تازه و اجرای موتور تحلیل..."):
        try:
    ...
    st.caption(
        f"منبع داده: **{result.data_source}** | "
        f"کندل‌های معتبر: {result.data_quality.total_candles} | "
        f"زمان اجرا: {run_time}"
    )
